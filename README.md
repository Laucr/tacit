...
### Verification
- **[bailiff](./bailiff/SKILL.md)** — Spec-driven verification with contract-level testing. Validates implementations against specifications, runs Go quality checks, and scans the tree (including `.claude/` / `.agent/`) for leaked local paths, secrets, and leftover unchosen-path comments.
