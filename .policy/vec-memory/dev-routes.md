...
Recall first at task start, save memory at the end with consent.
...
| When | Skill | Purpose |
|---|---|---|
| Before any task | `honcho-recall` | Pull standing prefs, past decisions, project context from long-term memory. |
| After any task | `honcho-remember` (with consent) | Distill new observations and persist — match user tone. |
| First time on a repo | `warmup` | Onboard, extract conventions into `.claude/rules/`, write `.claude/memory/current.md`. |
| Memory plumbing | `honcho-manage` | Setup, health-check, session scoping, deletion. |
...
└── no
    ...
    ├─ user prefs / past context?           ──▶ honcho-recall
